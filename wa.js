  ...
  return new Promise(async (resolve, reject) => {
    try {
      ...
      if (!sock.authState.creds.registered) {
        ...
        console.log('Existing session found for ' + phoneNumber + ' — reconnecting');
        sessions.set(phoneNumber, sock);
        resolve(null);
      }
    ...
    if (action === 'add' && global.welcomeMessages?.[id]) {
      for (const jid of participants) {
        ...
        await sock.sendMessage(id, {
          text: global.welcomeMessages[id].replace('{name}', '@' + name),
          mentions: [jid],
        });
      }
    }
...
async function handleCommand(cmd, args, msg, from, sock, reply) {
  ...
  switch (cmd) {
    ...
    case 'kick':
      ...
      if (!target) return reply('Tag the person to kick.');
      try {
        await sock.groupParticipantsUpdate(from, [target], 'remove');
        return reply(target.split('@')[0] + ' has been removed.');
      } catch { return reply('Could not kick — make sure RIAS is an admin.'); }

    case 'ban':
      if (!isGroup) return reply('Groups only.');
      if (!await isAdmin()) return reply('Admins only.');
      if (!target) return reply('Tag the person to ban.');
      try {
        await sock.groupParticipantsUpdate(from, [target], 'remove');
        return reply(target.split('@')[0] + ' banned. No returns.');
      } catch { return reply('Could not ban — make sure RIAS is an admin.'); }
    ...
    case 'mute':
      ...
      } catch { return reply('Could not mute — make sure RIAS is an admin.'); }
    ...
    case 'unmute':
      ...
      } catch { return reply('Could not unmute.'); }

    case 'promote':
      if (!isGroup) return reply('Groups only.');
      if (!await isAdmin()) return reply('Admins only.');
      if (!target) return reply('Tag the person to promote.');
      try {
        await sock.groupParticipantsUpdate(from, [target], 'promote');
        return reply(target.split('@')[0] + ' is now an admin.');
      } catch { return reply('Could not promote — make sure RIAS is an admin.'); }

    case 'demote':
      if (!isGroup) return reply('Groups only.');
      if (!await isAdmin()) return reply('Admins only.');
      if (!target) return reply('Tag the person to demote.');
      try {
        await sock.groupParticipantsUpdate(from, [target], 'demote');
        return reply(target.split('@')[0] + ' admin rights revoked.');
      } catch { return reply('Could not demote.'); }
    ...
    case 'lockgroup':
      ...
      } catch { return reply('Could not lock — make sure RIAS is an admin.'); }
    ...
    case 'unlockgroup':
      ...
      } catch { return reply('Could not unlock.'); }

    case 'warn': {
      if (!isGroup) return reply('Groups only.');
      if (!await isAdmin()) return reply('Admins only.');
      if (!target) return reply('Tag the person to warn.');
      ...
      if (count >= 3) {
        ...
        return reply(target.split('@')[0] + ' hit 3 warnings and has been removed.');
      }
      return reply('Warning ' + count + '/3 for ' + target.split('@')[0] + '. One more and they are out.');
    }
    ...
    case 'rank': {
      ...
      } catch { return reply('Could not fetch group members.'); }
    }

    case 'setwelcome': {
      if (!await isAdmin()) return reply('Admins only.');
      const wMsg = args.join(' ');
      if (!wMsg) return reply('Provide a welcome message. Use {name} for the member name.');
    ...
    case 'setgoodbye': {
      ...
      if (!bMsg) return reply('Provide a goodbye message.');
    ...
    case 'remind': {
      ...
      if (!ms || !message) return reply('Usage: .remind [1m/1h/1d] [message]');
      setTimeout(() => sock.sendMessage(from, { text: 'Reminder: ' + message }), ms);
      return reply('Reminder set for ' + timeStr + '.');
    }
    ...
    case 'schedule': {
      ...
      const message = args.slice(1).join(' ');
      const ms      = parseTime(timeStr);
      if (!ms || !message) return reply('Usage: .schedule [1m/1h/1d] [message]');
    ...
    case 'dm': {
      ...
      if (!dmTarget || !dmMsg) return reply('Usage: .dm @user [message]');
...
function getTruth() {
  const t = [
    ...
    "Growth is uncomfortable. That feeling right now? That's progress.",
  ];
  return 'Truth: ' + t[Math.floor(Math.random() * t.length)];
}
...
function getMenu() {
  return [
    ...
    '.promote .demote .lockgroup .unlockgroup .warn',
    '',
    'AI CHAT',
    '.ask [q]  .chat [msg]  .roast  .compliment',
    '.advice [topic]  .story [prompt]',
    '',
    'FUN',
    '.ship [@] [@]  .truth  .dare  .rank  .quote  .trivia',
    '',
    'AUTOMATION',
    '.setwelcome [msg]  .setgoodbye [msg]',
    '.antilink on/off  .antibadword on/off',
    '.remind [1m/1h/1d] [msg]  .schedule [time] [msg]',
    '',
    'DM TOOLS',
    '.dm @user [msg]  .broadcast [msg]',
    '.autoreply on/off  .savecontact @user',
    '',
    'Made by Jinx — RIAS bot',
  ].join('\n');
}
