...
_She doesn't beg for attention. She commands it. 🔴_`;
  ...
  // 3 real tappable buttons — WhatsApp max is 3
  const buttons = [
    { buttonId: 'ownermenu',  buttonText: { displayText: '👑 Owner Menu'  }, type: 1 },
    { buttonId: 'adminmenu',  buttonText: { displayText: '⚔️ Group Menu'  }, type: 1 },
    { buttonId: 'allcmds',    buttonText: { displayText: '📋 More'        }, type: 1 },
  ];

  try {
    await sock.sendMessage(from, {
      image:       { url: 'https://files.catbox.moe/h0flrc.jpg' },
      caption,
      footer:      '🔴 RIAS AI • Made by Jinx Official',
      buttons,
    ...
    // Fallback: plain image + caption if buttons fail
    try {
      await sock.sendMessage(from, {
        image:    { url: 'https://files.catbox.moe/h0flrc.jpg' },
        caption:  caption + '\n\n👑 *.ownermenu* | ⚔️ *.adminmenu* | 📋 *.allcmds*',
        mentions: [senderJid],
      }, { quoted: msg });
    } catch {
      await sock.sendMessage(from, {
